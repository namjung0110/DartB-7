...
주문 ID 기준이 아니라 행 단위로 계산되기 때문이다.
 하나의 주문(Order ID)이 여러 행으로 나뉘어 있을 경우
...
~~~
주문 ID 기준으로 계산이 이루어지도록 LOD(Level of Detail) 표현식을 사용해야 한다.
{ FIXED [Order ID] : DATEDIFF('day', MIN([Order Date]), MAX([Ship Date])) }
이렇게 하면 하나의 주문에 대해 정확한 배송 소요 일수를 계산할 수 있으며
행 단위로 계산되면서 발생하는 값 왜곡 문제를 해결할 수 있다.
~~~
